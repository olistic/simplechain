...
describe('Blockchain', () => {
  ...
  test('knows what the last block is', () => {
    const block = new Block(0, null, 0, 'foo');
  ...
  test('allows adding a valid block to the chain', () => {
    ...
    const block = new Block(index, lastBlockHash, timestamp, 'foo');
  ...
  test('rejects invalid blocks', () => {
    const lastBlock = blockchain.getLastBlock();
    const invalidBlock = new Block(0, null, 0, 'foo');
  ...
  describe('with first block different than genesis block', () => {
    test('is not valid', () => {
      blockchain.chain = [new Block(0, null, 0, 'foo')];
      expect(blockchain.isValid()).toBe(false);
    });
  });
  ...
  describe('with invalid block', () => {
    test('is not valid', () => {
      blockchain.chain.push(new Block(0, null, 0, 'foo'));
      expect(blockchain.isValid()).toBe(false);
    });
  });
});
